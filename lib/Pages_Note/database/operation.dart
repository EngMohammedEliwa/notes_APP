...
class OperationdataInsert {
  ...
  InsertData(String title, String note, int passedColor) async {
  ...
  UpdataData(String title, String note, int passedColor , int id) async {
  //  var  response = await database.updateData(''' UPDATE notes SET title = "${title}", note  = "${note}",color = "${passedColor}" WHERE id = ${id} ''');
  //  print(response);

    print(id);
    print('/////////////////////////////////////////');
    int count = await database.updateData(title, note,passedColor,id);
