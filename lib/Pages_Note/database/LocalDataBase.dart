...
class SqlDb {
  ...
  intialDb() async { // هنا بدي ابدأ انشاء قاعدة البيانات لكن اول اشي بدي اجيب المسار الي بدي احفظ فيه القاعدة داخل الجهاز فهنا بجيب المسار وضيفه للاسم عشان اعمل قاعدة البيانات
    String databasepath = await getDatabasesPath() ; // هنا بجيب المسار الافتراضي لحفظ قاعدة البيانات \اخل الجهاز الي بشغل البرنامج
  ...
  _onCreate(Database db , int version) async {
    await db.execute('CREATE TABLE notes (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, note TEXT, color INTEGER)');
    /*
  ...
  updateData(String title, String note, int passedColor , int id) async { // وهنا ايضا بترجع السطر الذي تم تحديثه
    Database? mydb = await db ;
    print( title);
    print( note);
    print(passedColor );
    ...
    int  response = await  mydb!.rawUpdate(
        'UPDATE notes SET id = ?, title = ?, note = ?, color = ? WHERE id = ?',
        [id ,title, note, passedColor,id]);
